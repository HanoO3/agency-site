  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 sm:px-8">
        ...
        <Link
          href="/"
          className="font-display text-lg sm:text-xl font-black uppercase tracking-tight text-[#F5F5F7] hover:text-[#E0432B] transition-colors flex items-center gap-1.5"
        >
          <span>{SITE_NAME}</span>
        ...
        <ul className="hidden md:flex items-center gap-10 text-[11px] font-mono uppercase tracking-[0.25em] text-white/60">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="nav-link-hover hover:text-[#F5F5F7] transition-colors py-1 inline-block"
              >
                {link.label}
              </Link>
            </li>
        ...
        <button
          className="md:hidden text-white/80 p-2 text-xl focus:outline-none"
      ...
      {/* Mobile Drawer */}
      {open && (
        <div className="md:hidden px-6 py-8 flex flex-col gap-6 bg-[#05050A]/95 border-b border-white/10 backdrop-blur-2xl">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setOpen(false)}
              className="text-white/80 hover:text-[#E0432B] transition-colors text-xs font-mono uppercase tracking-[0.25em]"
            >
              {link.label}
            </Link>
          ))}
